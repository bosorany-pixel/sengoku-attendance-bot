...
async def pov_stats(interaction: discord.Interaction):
    ...
    try:
        ...
        zero_pov = db_worker.fetchall(
            f"""SELECT uid, {_DISPLAY_NAME_SQL} AS display_name
                FROM USERS
                WHERE COALESCE(pov_count, 0) = 0 and server_username is not null
                ORDER BY display_name""",
            (),
        )
        # Users with last_pov older than 7 days (and have at least one POV)
        week_ago = db_worker.fetchall(
            f"""SELECT uid, {_DISPLAY_NAME_SQL} AS display_name, last_pov
                FROM USERS
